...
async function msk(json, file) {
    let tb = json.filter(item => item._8 === 'Подразделения МБ' ||
        item._8 === 'Руководство территориального банка' &&
        item._20 !== 'ВСП');
    ...
    let tb_upr = json.filter(item => item._8 === 'Руководство территориального банка');
    let tb_lvl3 = tb.filter(item => item._11 === 'Не присвоено' && item._8 === 'Подразделения МБ' && item._10 !== 'Не присвоено');
    ...
    for (let dep of tb_lvl3) {
        await dep_parse(final_lvl3_tb, dep, 3, 'tb', '_6',7)
    }
    ...
    for (let dep of lvl3.rows) {
        await recursive_children_fill(dep.depname, dep.lvl, dep.funcblock, dep.bank, dep.id, tb, file, 'tb', '_6',7)
    }
}
