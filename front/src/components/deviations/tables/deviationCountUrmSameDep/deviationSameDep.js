...
export default function DeviationUrm(props) {
    ...
    const buttonClick = () =>{
        setRequest(true);
        axios.get('deviation_count_urm_samedep/same_dep_update',).then(res => {
            setRequest(false);
        })
    };
    ...
    useEffect(() => {
        if(type.length > 0) {
            setLoading(true);
            axios.post('deviation_count_urm_samedep/samedep',{filter, type}).then(res => {
    ...
    const setFootNote = (id) => {
        axios.post('deviation_count_urm_samedep/setfootsamedep',{id}).then(res => setNoteArr(res.data));
    };

    useEffect(() => {
        axios.get('deviation_count_urm_samedep/getfootsamedep',{}).then(res => setNoteArr(res.data));
    },[]);
