...
export default React.memo(function ErrorDialog(props) {
    ...
    const {errorForm, setErrorForm, id, setItemErrors} = props;

    const [errorList, setErrorList] = React.useState([]);
    const [itemConnectedErrors, setItemConnectedErrors] = React.useState([]);
    ...
    React.useEffect(() => {
        Axios.post('match/error_list', {id}).then((response) => {
    ...
    React.useEffect(() => {
        if (redact !== null) {
            Axios.post('match/error_to_redact', {redact}).then((res) => {
    ...
    const submitNewError = () => {
        if (newErrorName.length > 0) {
            Axios.post('match/error_list_add', {newErrorName, newErrorDescription}).then((response) => {
                setErrorList(response.data.error_list);
            });
        }
        setAddError(false);
    ...
    const redactError = () => {
        if (newErrorName.length > 0) {
            Axios.post('match/error_redact', {redact, newErrorName, newErrorDescription}).then((response) => {
                setErrorList(response.data.error_list);
            });
        }
        setRedact(null);
    ...
    const deleteError = () => {
        if (redact !== null) {
            Axios.post('match/error_delete', {redact}).then((response) => {
                setErrorList(response.data.error_list);
            });
        }
        setRedact(null);
    ...
    const error_list = errorList.map((item, index) => <ErrorItem
        item={item}
        itemConnectedErrors={itemConnectedErrors}
        setItemConnectedErrors={setItemConnectedErrors}
        id={id}
        key={index}
